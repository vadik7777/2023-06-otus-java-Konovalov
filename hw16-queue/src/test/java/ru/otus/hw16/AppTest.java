...
class AppTest {
    ...
    void setUp() {
        ...
        SensorsDataQueueChannel sensorsDataChannel =
                new SensorsDataQueueChannel(SENSORS_DATA_QUEUE_CAPACITY);
        ...
        fakeSensorDataGenerator =
                new FakeSensorDataGenerator(randomGenerator, sensorsDataServer, SENSORS_COUNT);
    ...
    void shouldInvokeCorrectProcessorsAccordingItsBindings() throws InterruptedException {
        ...
        doAnswer(
                        a ->
                                (randomIntInvocationsCount.incrementAndGet() % 2 == 0)
                                        ? BINDED_ROOM_NUMBER
                                        : a.callRealMethod())
        ...
        var allData =
                allDataByRooms.entrySet().stream()
        ...
        ArgumentCaptor<SensorData> argumentCaptorForProcessorCommon =
                ArgumentCaptor.forClass(SensorData.class);
        verify(sensorDataProcessorCommon, times(allData.size()))
                .process(argumentCaptorForProcessorCommon.capture());
        assertThat(argumentCaptorForProcessorCommon.getAllValues())
                .containsExactlyInAnyOrderElementsOf(allData);

        ArgumentCaptor<SensorData> argumentCaptorForProcessorRoom =
                ArgumentCaptor.forClass(SensorData.class);
